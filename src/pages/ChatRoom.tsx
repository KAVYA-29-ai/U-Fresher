...
const ChatRoom = ({ roomId, onBack }: ChatRoomProps) => {
      ...
      <div key={message.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-4`}>
        <div className={`flex gap-3 max-w-[70%] ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
          ...
          <div className={`space-y-1 ${isOwn ? 'items-end' : 'items-start'}`}>
            {!isOwn && (
              <p className="text-xs text-slate-500 font-medium">{message.sender?.name || 'Unknown User'}</p>
            )}
            <div
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      {/* Header */}
      <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg shadow-blue-500/10 rounded-none">
          ...
          <div className="flex items-center justify-center h-full">
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl shadow-blue-500/10 rounded-2xl">
              <CardContent className="text-center py-12">
                ...
                <p className="text-slate-600 mb-4">Start the conversation by sending a message</p>
              </CardContent>
            </Card>
          </div>
        )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 relative">
              <Input
                ...
                className="pr-12 bg-slate-50/50 border-slate-200 rounded-2xl"
                disabled={sending}
              />
              <Button
                variant="ghost"
                size="sm"
                className="absolute right-2 top-1/2 transform -translate-y-1/2 text-slate-500 hover:text-blue-600"
              >
